...
(Unfortunately, I know of no way to automate this process through scripting.)
...
| grep "browser_download_url.*ubnautica-Mod-Setup-and-Install-scripts-for-Linux.tar.gz" \
| cut -d : -f 2,3 \
| tr -d \" \
| wget -qi -
tar –xf Subnautica-Mod-Setup-and-Bulk-Unzipper-scripts-for-Linux.tar.gz
rm -f Subnautica-Mod-Setup-and-Bulk-Unzipper-scripts-for-Linux.tar.gz
chmod +x install.sh
...
As an alternative to the command, you can download your preferred release [here](https://github.com/lunanerdderg/Subnautica-Mod-Setup-and-Bulk-Unzipper-scripts-for-Linux/releases/tag/latest), unzip it, and run `chmod +x` on both shell scripts.
...
5. *Keep the terminal window open.* Download all the mods you want to install. (Make sure to keep track of dependencies! *On Nexus in a mod's "Description" tab, clicking the "Requirements" button will reveal them.*)
...
6. Place all the mods in a single folder if you want to keep the .zips, or place them in `~/.local/share/Steam/steamapps/common/Subnautica/BepInEx/plugins/` if you do not.
7. Run `unzip-mods-in-bulk.sh` in the terminal (again, ideally the same terminal as before) with the location of your mod folder as an argument, or leave blank if you placed your mods in `~/.local/share/Steam/steamapps/common/Subnautica/BepInEx/plugins/`.
...
* Your project uses the same license
